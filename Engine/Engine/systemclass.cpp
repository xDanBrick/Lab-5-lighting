...
bool SystemClass::Initialize()
{
	...
	result = m_Graphics->Initialize(screenWidth, screenHeight, &gameWindow_);
	if(!result)
	{
		return false;
	}
...
LRESULT SystemClass::MainMessageHandler(HWND hwnd, UINT umsg, WPARAM wparam, LPARAM lparam)
{
	switch (umsg)
	{
		case WM_PAINT:
		{
			...
			ApplicationHandle->m_Graphics->OnPaint(hdc);
			EndPaint(hwnd, &ps);
			return 0;
		}
		case WM_COMMAND:
		{
			// If a key is pressed send it to the input object so it can record that state.
			ApplicationHandle->m_Graphics->OnCommand(wparam);
			return 0;
		}
		// Any other messages send to the default message handler as our application won't make use of them.
		default:
...
void SystemClass::InitializeWindows(int& screenWidth, int& screenHeight)
{
	...
	gameWindow_.Show(SW_SHOW);
	gameWindow_.setForeground();
